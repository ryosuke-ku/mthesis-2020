...
\begin{description}
  \item[行単位の検出] この検出手法では，ソースコードを行単位で比較し，閾値以上の長さで重複している行をコードクローンとして検出する．
  \item[字句単位の検出] 字句単位の検出手法では，検出の前処理としてソースコードを字句の列に変換する．そして，閾値以上の長さで一致している字句の部分列をコードクローンとして検出する．
  \item[抽象構文木を用いた検出] 抽象構文木とは，ソースコードの構文構造を木構造で表したグラフのことを意味する．この検出手法では，検出の前処理としてソースコードに対して構文解析を行うことによって，抽象構文木を構築する．そして，抽象構文木上の同形の部分木をコードクローンとして検出する．
\end{description}
